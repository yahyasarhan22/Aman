...
async function seed() {
  ...
  const nourBakery = await establishmentRepo.save({
    slug: 'nour-bakery',
    nameAr: 'مخبز النور',
    nameEn: null,
    category: 'BAKERY',
  ...
  await userRepo.save({
    email: 'owner@nour-bakery.ps',
    passwordHash: await hashPassword('aman1234'),
    role: 'OWNER',
    displayNameAr: 'صاحب مخبز النور',
    establishmentId: nourBakery.id,
  });
  ...
  console.log(
    `Seeded ${establishmentCount} establishments, ${items.length} checklist items, ${complaintCount} complaints, 4 users.`,
  );
  ...
  console.log('Owner:     owner@nour-bakery.ps / aman1234');
