...
@Component({
  ...
  styles: [
    `
      ...
      svg {
        inline-size: 100%;
        /* This chart draws in its own fixed coordinate space regardless of
           page direction (see class doc) — without an explicit ltr here,
           text-anchor="end" inherits the page's dir="rtl" and some browsers
           flip which side "end" grows toward, clipping text off the canvas. */
        direction: ltr;
      }
      .label {
        font-size: 12px;
...
export class BarChartComponent {
  ...
  /** Deliberately short of half the canvas — the value label sits just
   *  before the bar's start, so even a bar at 100% of the max needs room
   *  left over on the canvas for its own number instead of running off it. */
  readonly barMax = 110;
