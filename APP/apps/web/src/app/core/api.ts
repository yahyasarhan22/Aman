...
// Relative — the dev server proxies /api and /uploads to the Nest server
// (see proxy.conf.json), so the browser only ever talks to one origin. That
// keeps this correct through a tunnel too, where the API's own port/host
// isn't reachable at all, only whatever origin serves the page.
