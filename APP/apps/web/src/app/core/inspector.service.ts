...
export interface BundleComplaint {
  reference: string;
  category: string;
  description: string;
  hasEvidence: boolean;
...
export class InspectorService {
  ...
  getCompletedToday(): Promise<CompletedTodayEntry[]> {
    return firstValueFrom(
      this.http.get<CompletedTodayEntry[]>(`${API_BASE}/api/inspector/completed-today`, {
        headers: this.auth.authHeaders(),
      }),
    );
  }

  getInspectionDetail(inspectionId: string): Promise<InspectionDetail> {
    return firstValueFrom(
      this.http.get<InspectionDetail>(`${API_BASE}/api/inspector/inspections/${inspectionId}`, {
        headers: this.auth.authHeaders(),
      }),
    );
  }
  ...
  async startInspection(establishmentId: string): Promise<InspectionDraft> {
    ...
    // A draft saved before `complaints` shipped on the bundle carries a stale
    // one — refresh it in place rather than leaving the inspector's context
    // panel silently short of what a fresh visit would show.
    if (existing) {
      if (existing.bundle.complaints) return existing;
      const bundle = await this.fetchBundle(establishmentId);
      const refreshed = { ...existing, bundle };
      await this.saveDraft(refreshed);
      return refreshed;
    }
  ...
  private fetchBundle(establishmentId: string): Promise<EstablishmentBundle> {
    return firstValueFrom(
